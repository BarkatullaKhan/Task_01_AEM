...
import javax.servlet.ServletException;import org.apache.sling.api.SlingHttpServletRequest;
...
"sling.servlet.paths=" + "/aemApp/bin/api/content/v3",
"sling.servlet.extensions=" + "json"})
public class RestServletV3 extends SlingSafeMethodsServlet {
	...
	private void iterate(NodeIterator nodeItr,PrintWriter out,String strpath)
		...
		out.write("{");
			...
			if (cNode.getName().equals("responsivegrid")) {
				nodeItr=cNode.getNodes();
				continue;
...
//			  propertiesToIgnore.add("jcr:title");
			...
			try {
				...
				if(nodeItr.hasNext()){
					out.write("\""+cNode.getName()+"\""+":"+jsonObject.toString()+",");
				}else{
					out.write("\""+cNode.getName()+"\""+":"+jsonObject.toString());
				}
